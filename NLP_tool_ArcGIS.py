...
def netowl_curl(infile, outpath, outextension, sess):
    ...
    #                          verify=False)
    response = sess.post('https://api.netowl.com/api/v2/_process',
                             headers=headers, params=params, data=data,
                             verify=False)
...
# wk = arcpy.env.workspace = r"C:\Users\heat3463\Documents\ArcGIS\Packages\Netowl_Pro_pkg_E5FCF549-D8FE-4531-BF5B-D9B0EDB80DC8\p20\netowl.gdb"  # noqa: E501)
wk = arcpy.env.workspace = r"C:\Users\heat3463\Documents\ArcGIS\Packages\Netowl_Pro_test_0B846E0E-389F-490E-A753-F83D46F0DE74\p20\netowl.gdb"
...
#     netowl_curl(d, rdfOutDir, rdfOutExt, session)

# ---------------------------------------
# End comment for API consideration
# ---------------------------------------
...
# ----------------------------------
        for e in ents:
            ...
            rdfvalue = e['value']
            #cleanup_text(e['value'])  # value (ie name)
            print(e['value'])
            rdfid = e['id']
...
# ----------------------------------
#     Write out nodes and links 
# ----------------------------------

import csv
with open('nodes.csv', 'w', newline='') as csvfile:
    spamwriter = csv.writer(csvfile)
    spamwriter.writerow(["id", "value", "orgdoc", "isGeo", "type"])
    for r in rdfobjsGeo:
        spamwriter.writerow([r.id, r.value, r.orgdoc, "yes", r.type])
    for u in rdfobjs:
        spamwriter.writerow([u.id, u.value, u.orgdoc, "no", u.type])
...
# arcpy.AddRelate_management('netowl_output', 'ORGDOC', 'netowl_output', 'ORGDOC', "Locations_from_same_document", 'MANY_TO_MANY')

# arcpy.AddRelate_management('netowl_links', 'TOID', 'netowl_output', 'UNIQUEID', "links_to_output", 'MANY_TO_MANY')
# arcpy.AddRelate_management('netowl_links', 'FROMID', 'netowl_output', 'UNIQUEID', "links_from_output", 'MANY_TO_MANY')

# arcpy.AddRelate_management('netowl_links', 'FROMID', 'netowl_entities', 'UNIQUEID', "links_from_entities", 'MANY_TO_MANY')
# arcpy.AddRelate_management('netowl_links', 'TOID', 'netowl_entities', 'UNIQUEID', "links_to_entities", 'MANY_TO_MANY')

# arcpy.AddRelate_management('netowl_events', 'FROMID', 'netowl_output', 'UNIQUEID', 'events_from_locations', 'MANY_TO_MANY')
# arcpy.AddRelate_management('netowl_events', 'TOID', 'netowl_output', 'UNIQUEID', 'events_to_locations', 'MANY_TO_MANY')

# arcpy.AddRelate_management('netowl_events', 'FROMID', 'netowl_entities', 'UNIQUEID', 'events_from_entities', 'MANY_TO_MANY')
# arcpy.AddRelate_management('netowl_events', 'TOID', 'netowl_entities', 'UNIQUEID', 'events_to_entities', 'MANY_TO_MANY')

#------------------------------
# end create relationship class
#------------------------------
